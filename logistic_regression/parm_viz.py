...
# Constrain solver choices based on penalty and multi_class to avoid invalid combos
...
if multi_class == 'multinomial':
    ...
    solver_options = [s for s in solver_options if s != 'liblinear']

solver = st.sidebar.selectbox(
    'Solver',
    tuple(solver_options)
)

max_iter = int(st.sidebar.number_input('Max Iterations', value=100))

# l1_ratio only applies for elasticnet and must be float in [0, 1]
l1_ratio = None
if penalty == 'elasticnet':
    l1_ratio = float(st.sidebar.slider('l1 Ratio', min_value=0.0, max_value=1.0, value=0.5))
